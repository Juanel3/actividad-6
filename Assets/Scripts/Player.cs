...
{
    GameObject flash = Instantiate(disparo, shootPoint.position, shootPoint.rotation);
    flash.GetComponent<Light>().enabled = true; // Activa la luz
    yield return new WaitForSeconds(0.1f); // Duración del destello
    Destroy(flash); // Elimina el destello después de 0.1 segundos
}
